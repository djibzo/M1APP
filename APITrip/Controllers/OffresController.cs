...
namespace APITrip.Controllers
{
    ...
    public class OffresController : ControllerBase
    {
        ...
        [HttpPost]
        public IActionResult Create(CreateRequest model) { _service.Create(model); return Ok(new { message = "Offre created" }); }
        [HttpPut("{id}")]
        public IActionResult Update(int id, UpdateRequest model) { _service.Update(id, model); return Ok(new { message = "Offre updated" }); }
        [HttpDelete("{id}")]
